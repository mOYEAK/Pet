...
> 文档快照：2026-07-15  
> 适用范围：当前工作区代码，包含尚未提交的“经营数据统计与报表”改动。  
...
- [ ] 生产级日志、监控、告警、CI/CD 和自动备份。
- [ ] 项目自有单元测试、API 集成测试和浏览器 E2E 测试。
...
当前仓库没有项目自有的 `*.spec.ts`、`*.test.ts` 或 Playwright/Cypress E2E 用例。因此：

- 当前已经完成的是类型检查、生产构建、API 场景测试和浏览器手工验收。
- 每次大改仍需要人工重复执行关键闭环。
- 数据事务回归主要依赖手工 API 调用和数据库查询。
- 下一阶段应优先为预约冲突、订单支付、优惠券核销、充值、套餐卡和统计口径增加自动化测试。
