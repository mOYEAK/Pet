...
> 文档快照：2026-07-19
> 适用范围：当前工作区代码，包含核心业务 API 自动化测试、Playwright 浏览器 E2E 与 CI。
...
- [x] Playwright 浏览器 E2E，覆盖 C 端预约、后台确认、生成订单、优惠券支付和 C 端结果回显。
- [x] GitHub Actions 持续执行集成测试、覆盖率、全仓 typecheck 和三端 build。
- [△] 系统设置目前为代码中的固定配置，只读展示，不能在后台保存修改。
- [ ] 生产级日志、监控、告警和自动备份。
- [△] API 集成测试和浏览器 E2E 已完成，细粒度单元测试尚未补充。
...
| 检查项         | 通过条件                                                        | 结果       | 证据                           |
| -------------- | --------------------------------------------------------------- | ---------- | ------------------------------ |
| Migration      | 数据库为最新版本                                                | 已通过     | 演示库 6 个 migration 均为最新 |
| Typecheck      | `pnpm typecheck` 退出码 0                                       | 已通过     | 终端输出                       |
| API build      | API build 退出码 0                                              | 已通过     | 终端输出                       |
| Admin build    | 后台 build 退出码 0                                             | 已通过     | 终端输出                       |
| Miniapp build  | H5 build 退出码 0                                               | 已通过     | 终端输出                       |
| C 端预约       | 创建、冲突、取消全部符合预期                                    | 已通过     | API 测试、E2E、浏览器截图      |
| 后台预约       | 确认、手工创建、日历符合预期                                    | 已通过     | E2E、演示数据、浏览器截图      |
| 订单支付       | 金额、状态、`paidAt`、流水正确                                  | 已通过     | API 测试、E2E、浏览器截图      |
| 优惠券         | 门槛、核销、防重复正确                                          | 已通过     | API 测试、E2E、浏览器截图      |
| 会员权益       | 充值、余额、套餐次数正确                                        | 已通过     | API 测试、会员中心截图         |
| 通知           | 触发、未读、提醒、防重复正确                                    | 已通过     | API 测试、通知中心浏览器验收   |
| 报表           | 三周期、图表、统计口径正确                                      | 已通过     | API 测试、经营报表截图         |
| AI 降级        | 无密钥仍可使用                                                  | 已通过     | API 测试、C 端浏览器问答       |
| 真实 AI        | 工具调用和草案校验正确                                          | 可选未验收 | 需要有效模型密钥               |
| API 自动化测试 | `pnpm test:api` 的 16 个用例全部通过                            | 已通过     | Jest 3/3、16/16                |
| 浏览器 E2E     | `pnpm test:e2e` 完成预约、确认、订单、优惠券支付与回显          | 已通过     | Playwright 1/1                 |
| CI             | API 覆盖率、浏览器 E2E、typecheck 和三端 build 均配置为必过步骤 | 已通过     | GitHub Actions #2，3 分钟      |
| Docker         | 完整容器部署可访问                                              | 环境受限   | 构建后端卡住，详见部署验收记录 |

只有“构建检查 + API/数据库检查 + 关键手工闭环”全部通过，才能判定当前 MVP 功能真正完成。仅看到页面或仅通过 TypeScript 编译都不够。

本轮最终验收采用“隔离数据库自动化写入 + 演示库浏览器只读检查”的组合：会改变状态的预约、支付、优惠券和会员事务在 `5433/petcare_test` 验证；`5432/petcare` 只用于页面、报表、通知和 AI 降级展示，未重新 seed 或人为制造测试垃圾数据。8 张截图和两段录屏见 [showcase-assets.md](showcase-assets.md)。
...
- `pnpm test:e2e` 使用 Playwright Chromium 真实联动 C 端 H5、商家后台、API 与隔离 PostgreSQL，覆盖完整预约支付主链路。
- GitHub Actions 会持续执行覆盖率测试、浏览器 E2E、全仓 typecheck、API build、后台 build 和 C 端 H5 build。
- 当前浏览器自动化聚焦一条核心成功路径；其他页面交互、视觉布局、异常分支和真实 LLM 仍需要手工验收或继续扩展用例。
